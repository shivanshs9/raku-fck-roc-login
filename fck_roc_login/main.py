...
dotenv.load_dotenv(dotenv_path=os.getenv("ROC_ENV_FILE"))
...
class RocLoginMethod(object):
    ...
    async def fill_creds(self):
        try:
            ...
            input_password = await asyncio.to_thread(
            ...
            raise Exception("Failed to fill creds") from e
            ...
            try:
                is_auth = await AsyncWebDriverWait(self.driver).until(
                    EC.text_to_be_present_in_element(
                        (By.TAG_NAME, "body"), "Authenticated"
                    )
                )
...
async def login(cluster, callback_port=None):
    ...
    args = ["login", "-c", cluster, "--disable-browser"] + (
        ["--callback-port", str(callback_port)] if callback_port else []
    )
    try:
        process = await asyncio.create_subprocess_exec(
            CMD_ROC, *args, stdout=asyncio.subprocess.PIPE
        )
        ...
        logger.fatal(
            "Unable to read IAM login URL, maybe connection issue. Ensure VPN is connected!",
            exc_info=False,
        )
    finally:
        try:
